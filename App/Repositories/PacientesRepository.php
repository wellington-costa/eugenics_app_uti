...
class PacientesRepository
{
    ...
    public function pacientesInternadosMes($mes, $ano, $idHospital)
    {
        $query = $this->paciente->query(
            "SELECT COUNT(*) AS qtdPacientes FROM pacientes WHERE id_hospital = {$idHospital}
            AND MONTH(created_at) = '{$mes}' AND YEAR(created_at) = '{$ano}'
            AND status_now = 'Internado'"

        );

        return $query[0]->qtdPacientes;
    }
    ...
    {
        $query = $this->paciente->query(
            "SELECT COUNT(*) AS pacientesInternadosDia FROM vendas WHERE id_hospital = {$idHospital}
